...
from utils import Config, pexists, pjoin, safe_path_component, tenacity
...
def _load_json_cache(path: str):
    try:
        ...
        # Migrate caches created on Windows before UTF-8 was specified.
        with open(path, encoding="gb18030") as stream:
...
class SlideInducter:
    ...
    def layout_induct(self):
        ...
        if pexists(self.induct_cache):
            return _load_json_cache(self.induct_cache)
        ...
        if self.config.DEBUG:
            for layout_name, cluster in self.slide_induction.items():
                cluster_dir = pjoin(
                    self.output_dir,
                    "cluster_slides",
                    safe_path_component(layout_name),
                )
        ...
        json.dump(
            ...
            open(self.induct_cache, "w", encoding="utf-8"),
            indent=4,
            ensure_ascii=False,
        )
    ...
    def category_split(self):
        ...
        json.dump(
            ...
            open(self.split_cache, "w", encoding="utf-8"),
            indent=4,
            ensure_ascii=False,
        )
    ...
    def layout_split(self, content_slides_index: set[int]):
        ...
        template = Template(
            open("prompts/ask_category.txt", encoding="utf-8").read()
        )
    ...
    def content_induct(self):
        ...
        content_induct_prompt = Template(
            open("prompts/content_induct.txt", encoding="utf-8").read()
        )
        for layout_name, cluster in self.slide_induction.items():
