...
def safe_path_component(value: str) -> str:
    """Return a cross-platform-safe directory or file-name component."""
...
def _find_soffice() -> str:
    ...
    raise FileNotFoundError(
        "LibreOffice executable 'soffice' was not found. "
...
def _find_poppler_path() -> str | None:
    ...
    if configured and os.path.exists(os.path.join(configured, "pdfinfo.exe")):
        return configured

    pdfinfo = shutil.which("pdfinfo")
    if pdfinfo and pdfinfo.lower().endswith(".exe"):
        return os.path.dirname(pdfinfo)

    candidates = []
    if pdfinfo:
        dependencies_dir = Path(pdfinfo).resolve().parents[2]
        candidates.append(
            dependencies_dir / "native" / "poppler" / "Library" / "bin"
        )
    candidates.extend(
        [
...
def ppt_to_images(file: str, output_dir: str, warning: bool = False):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        profile_uri = Path(temp_dir, "lo_profile").resolve().as_uri()
        command_list = [
            _find_soffice(),
        ...
        for f in os.listdir(temp_dir):
            ...
            images = convert_from_path(
                temp_pdf, dpi=72, poppler_path=_find_poppler_path()
            )
