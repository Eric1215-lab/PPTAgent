...
MODEL_DEVICES = list(range(DEVICE_COUNT)) if DEVICE_COUNT else ["cpu"]
MODEL_DTYPE = torch.float16 if DEVICE_COUNT else torch.float32
REFINE_TEMPLATE = Template(
    open("prompts/document_refine.txt", encoding="utf-8").read()
)
...
text_models = [
    BGEM3FlagModel(
        "BAAI/bge-m3",
        use_fp16=bool(DEVICE_COUNT),
        device=MODEL_DEVICES[i % len(MODEL_DEVICES)],
    )
    for i in range(NUM_MODELS)
]
image_models = [
    get_image_model(device=MODEL_DEVICES[i % len(MODEL_DEVICES)])
    for i in range(NUM_MODELS)
]
marker_models = [
    create_model_dict(
        device=MODEL_DEVICES[i % len(MODEL_DEVICES)], dtype=MODEL_DTYPE
    )
    for i in range(NUM_MODELS)
]
...
async def feedback(request: Request):
    ...
    with open(f"runs/feedback/{task_id}.txt", "w", encoding="utf-8") as f:
        f.write(feedback)
...
def ppt_gen(task_id: str, rerun=False):
    ...
    json.dump(
        task,
        open(
            pjoin(generation_config.RUN_DIR, "task.json"),
            "w",
            encoding="utf-8",
        ),
    )
    ...
    if len(pdf_md5) != 32:
        ...
        if not os.path.exists(pdf_dir + "/refined_doc.json"):
            ...
            json.dump(
                ...
                open(pjoin(pdf_dir, "refined_doc.json"), "w", encoding="utf-8"),
            )
    ...
    try:
        ...
        if not os.path.exists(pjoin(parsedpdf_dir, "image_caption.json")):
            caption_prompt = open("prompts/caption.txt", encoding="utf-8").read()
            images = {}
            ...
            json.dump(
                images,
                open(
                    pjoin(parsedpdf_dir, "image_caption.json"),
                    "w",
                    encoding="utf-8",
                ),
                ensure_ascii=False,
            ...
            images = json.load(
                open(pjoin(parsedpdf_dir, "image_caption.json"), encoding="utf-8")
            )
        if not os.path.exists(pjoin(parsedpdf_dir, "refined_doc.json")):
...
def setup_models():
    if os.path.exists("serve.json"):
        serve_config = json.load(open("serve.json", encoding="utf-8"))
