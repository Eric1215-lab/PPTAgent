...
class Document:
    ...
    async def _parse_chunk_async(
        ...
        if retry == 0:
            medias = to_paragraphs(section) # 将当前块分割为段落，并识别其中的表格和图片，同时为每个表格/图片收集上下文段落。
            turn_id, section = await extractor(markdown_document=section) # 识别子章节，提取标题和内容并提取元数据
            metadata = section.pop("metadata", {}) # 从提取结果中分离元数据
        try:
            section["subsections"] = link_medias(medias, section["subsections"]) # 将表格、图片关联到重写后的最相关段落或子章节。
            section = Section.from_dict(section)
            for media in section.iter_medias():
