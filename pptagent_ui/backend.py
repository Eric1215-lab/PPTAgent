...
def load_json_file(path: str):
    """Read UTF-8 files while remaining compatible with legacy Windows caches."""
    try:
...
def read_text_file(path: str) -> str:
    """Read UTF-8 text while remaining compatible with legacy Windows caches."""
    try:
        with open(path, encoding="utf-8") as f:
            return f.read()
    except UnicodeDecodeError:
...
async def ppt_gen(task_id: str, rerun=False):
    ...
    if rerun:
        ...
        progress_store[task_id] = load_json_file(
            pjoin(RUNS_DIR, task_id, "task.json")
        )
    ...
    json.dump(
        task,
        open(
            pjoin(generation_config.RUN_DIR, "task.json"), "w", encoding="utf-8"
        ),
    )
    ...
    try:
        ...
        if not os.path.exists(pjoin(parsedpdf_dir, "source.md")):
            ...
            text_content = read_text_file(pjoin(parsedpdf_dir, "source.md"))
        await progress.report_progress()
        ...
        if not os.path.exists(pjoin(pptx_config.RUN_DIR, "slide_induction.json")):
            ...
            slide_induction = load_json_file(
                pjoin(pptx_config.RUN_DIR, "slide_induction.json")
            )
        await progress.report_progress()
